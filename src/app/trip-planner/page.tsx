...
// import { ItineraryDays, } from "@/types";
import {
...
const TripPlanner = () => {
  ...
  const createTripMutation = useMutation({
    mutationFn: async (tripData: any) => {
      ...
      const dataForApi = {
        destinations:
          tripData.destinations?.map((dest: any) => ({
            city: dest.location,
  ...
  useEffect(() => {
    if (tripData) {
      ...
      if (
        ...
        tripData.transportationOptions.length
      ) {
  ...
  const handleAddPreference = () => {
    if (
      ...
      !preferences.includes(selectedPreferenceInput)
    ) {
  ...
  const handleAddDestination = () => {
    if (newDestination && newDaysToStay > 0) {
      ...
      toast({
        ...
        description:
          "Please provide both a destination name and number of days to stay.",
        variant: "destructive",
      });
    }
  };
  ...
  const handleSetTransportationMode = (
    ...
    const existingOptionIndex = transportationOptions.findIndex(
      (opt) =>
        opt.fromDestination === fromIndex && opt.toDestination === toIndex
    );
  ...
  const handleGenerateItinerary = async () => {
    ...
    console.log("Current tripId:", tripId); // Add this log
    ...
    const validDestinations = destinations.filter((dest, index) => {
      ...
      return hasDestination && isString && hasContent;
    });
    ...
    try {
      ...
      // Create the request object - ALWAYS include tripId if it exists
      ...
      if (tripId) {
        itineraryRequest.tripId = tripId;
        console.log("Adding tripId to request:", tripId);
      } else {
        console.log("No tripId available - detailed itinerary won't be saved");
      }

      console.log(
        "Final itinerary request:",
        JSON.stringify(itineraryRequest, null, 2)
      );

      // Always use the same endpoint - it handles both scenarios
      console.log("=== CALLING GENERATE ITINERARY ENDPOINT ===");
      const response = await makeAuthenticatedApiRequest(
        "POST",
        "/api/generate-itinerary", // Always use this single endpoint
        itineraryRequest
      );
      ...
      // Update the main trip record with the itinerary (only if tripId exists)
      if (tripId) {
        ...
        console.log("=== TRIP RECORD UPDATED ===");
      }
      ...
      toast({
        ...
        description: tripId
          ? "Your personalized itinerary has been created and saved with detailed day-by-day information."
      ...
      toast({
        ...
        description:
          error.message || "There was a problem generating your itinerary.",
        variant: "destructive",
      });
    } finally {
  ...
  // Remove the old savedDetailedItinerary function since it's now handled by the backend
  ...
  const handleDeleteActivity = async (activity: Activity) => {
    ...
    if (tripId) {
      ...
      try {
        const dayToUpdate = updatedItinerary.days.find(
          (day) => day.day === selectedDayIndex
        );
        if (dayToUpdate) {
          // Find the corresponding ItineraryDay record
          const detailedDays = await makeAuthenticatedApiRequest(
            "GET",
            `/api/itinerary-days/trip/${tripId}`
          );
          const dayRecord = detailedDays.find(
            (d: any) => d.dayNumber === selectedDayIndex
          );

          if (dayRecord) {
            const updatedActivities = dayToUpdate.activities.map(
              (activity) => ({
...
// Updated handleBookTrip function in your TripPlanner component
const handleBookTrip = async () => {
  ...
  try {
    if (!tripId) {
      ...
      // NOW SAVE THE DETAILED ITINERARY using the new API endpoint
      console.log("=== SAVING DETAILED ITINERARY FOR NEW TRIP ===");
      ...
      toast({
        ...
        description: "Your trip has been booked and saved with detailed itinerary.",
      });
      ...
      // First update the main trip record
      await updateTripMutation.mutateAsync({
        id: tripId,
        data: {
          itinerary: generatedItinerary,
      ...
      // THEN SAVE THE DETAILED ITINERARY using the API endpoint
      console.log("=== SAVING DETAILED ITINERARY FOR EXISTING TRIP ===");
      await makeAuthenticatedApiRequest("POST", "/api/save-detailed-itinerary", {
        tripId: tripId,
        itineraryData: generatedItinerary,
      });
      ...
      toast({
        title: "Trip booked successfully!",
        description: "Your trip has been confirmed and saved with detailed itinerary.",
      });

      // Navigate to book trip page
